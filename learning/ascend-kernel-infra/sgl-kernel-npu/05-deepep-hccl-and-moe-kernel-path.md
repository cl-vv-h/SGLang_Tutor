...
本章的 `Buffer/Config/handle` 是 Host Python/pybind 对象，`x/topk_idx` 等是 NPU `torch.Tensor`，HCCL communicator 又是通信资源句柄。它们不是同一种“tensor 参数”；统一类型读法见[代码阅读手册](../reference/code-reading-and-types.md)。
...
## 13. 官方测试源码：拆开的 layout → dispatch → combine

下面改写自固定 commit 的 `tests/python/deepep/test_common.py::normal_test`，保留真实 API 与完整变量声明。这个通信测试故意把 `recv_x` 直接交给 combine，相当于用 identity 代替本地 expert 计算，从而单独验证路由和通信：
...
这段源码最值得记住的不是函数名，而是顺序：

`router 结果 -> layout -> dispatch -> local expert compute -> combine`

| 名字 | Python 层类型 | shape/dtype 或角色 |
|---|---|---|
| `num_tokens/hidden/num_experts/num_topk` | `int` | Host shape/配置标量 |
| `buffer` | `deep_ep.Buffer` | 包装通信 buffer 与运行时状态的 pybind 对象 |
| `config` | `deep_ep.Config` | dispatch/combine 调优配置，不是 tensor |
| `x` | NPU `torch.Tensor` | `[num_tokens,hidden]`, BF16 |
| `scores/topk_weights` | NPU `torch.Tensor` | FP32 router 值 |
| `topk_idx` | NPU `torch.Tensor` | `[num_tokens,num_topk]`，整数索引 |
| layout 命名返回值 | NPU tensor/接口定义对象 | rank/expert 计数与 membership map |
| `recv_x` | NPU tensor 或量化 tuple（依配置） | dispatch 后按本地 expert 排列的数据 |
| `handle` | tuple-like 路由元数据 | combine 恢复来源与权重所需，不能直接当数值 tensor 计算 |
| `combined_x` | NPU `torch.Tensor` | 恢复 token 顺序后的结果 |

真实 MoE 模型会在 dispatch 与 combine 之间把 `recv_x` 交给 grouped expert FFN；具体 FFN API、权重类型和量化格式由上层模型决定，不应在通信教程里用未声明的假函数冒充源码。
