...
移除 `--disable-cuda-graph` 后，ModelRunner 会创建 `NPUGraphRunner`。Graph 不会替换 GLM 模型类，也不会把 47 层合成一个新的数学算子；它记录一次固定 shape/address 下的 NPU task 提交序列，之后重复提交同一序列。

下面的主线默认**没有额外启用 `torch.compile`**，先单独观察 NPUGraph 的 capture/replay；如果两者同时开启，则是“编译后的程序图”被 NPUGraph 记录为“设备任务提交图”，两层图并不是同一个对象，见 13.4.4。

### 13.1 先纠正名字：源码叫 CUDA Graph，NPU 实际创建 NPUGraph

SGLang 最初以 CUDA 为主，后来复用了很多跨平台抽象名，所以在 NPU 路径中仍能看到：
...
这些名字不代表昇腾设备偷偷运行了 NVIDIA CUDA。当前 NPU 子类最终创建的是：
...
可以把 `CudaGraphRunner` 理解为 SGLang 的历史/跨平台接口名，把 `NPUGraph` 理解为 torch_npu 和 CANN 真正执行的设备机制。本章后文说“Graph”时，若没有特别声明，均指 NPU 上的 `torch.npu.NPUGraph`。

还要纠正“加载一张图”的直觉：

> 默认路径不是从模型目录读取一个预先保存的 `.graph` 文件。每个 Scheduler/TP rank 在服务启动阶段，用本 rank 已加载的 GLM 权重、静态输入 buffer、KV pool 和 NPU stream 现场执行 warmup 与 capture，得到只存在于当前进程/runtime 中的 `NPUGraph` 对象。

磁盘上可能存在 ATen/Triton/torch.compile 等 kernel 的编译缓存，但“加载 kernel binary”与“从磁盘加载整张 SGLang decode NPUGraph”是两回事。

### 13.2 启动时，Graph 在什么时候创建

固定源码中，[`Scheduler.init_model_worker()`](https://github.com/sgl-project/sglang/blob/9a03bebf13996b628f8335628a691dcb3aa8400b/python/sglang/srt/managers/scheduler.py#L901-L921) 的关键顺序是：

```text
init_tp_model_worker()
  -> 创建 TpModelWorker / ModelRunner
  -> 构造 GLM 模型并加载本 TP rank 的权重

init_memory_pools()
  -> 创建 request pool 与 paged MLA KV pool

init_all_attention_backends()
  -> 创建 AscendAttnBackend 及 graph metadata/buffer 能力

init_all_cuda_graphs()
  -> 名字仍叫 cuda graph
  -> NPU 设备选择 NPUGraphRunner
  -> warmup + capture
...
1. 先有 `Glm4MoeLiteForCausalLM` 对象和本 rank 权重；
2. 再有 KV cache、attention backend 和静态 buffer；
3. 然后执行真实 GLM forward 来捕获 Graph；
4. 最后在线请求才会 replay。

TP=4 时没有“一张中心图发给四张卡”。四个 TP rank 各自在自己的进程、NPU、current stream、local 权重和 local KV pool 上创建自己的 `NPUGraphRunner` 与图字典。四份图的程序结构相似，但绑定的是各 rank 的本地地址。

`ModelRunner.init_cuda_graphs()` 会进入 [`capture_cuda_graphs()`](https://github.com/sgl-project/sglang/blob/9a03bebf13996b628f8335628a691dcb3aa8400b/python/sglang/srt/model_executor/model_runner_components/cuda_graph_setup.py#L73-L130)。它依次建立：

```text
GraphSharedOutput
  -> EagerRunner
  -> 可选 prefill graph runner
  -> decode graph runner
```

对本章“无 speculative 的 GLM decode”场景，[`capture_decode_graph()`](https://github.com/sgl-project/sglang/blob/9a03bebf13996b628f8335628a691dcb3aa8400b/python/sglang/srt/model_executor/model_runner_components/cuda_graph_setup.py#L285-L365) 根据 `model_runner.device == "npu"` 选择 `NPUGraphRunner(model_runner)`。`NPUGraphRunner` 继承通用 `DecodeCudaGraphRunner`，只替换 NPU graph backend、NPU capture 和动态 attention 参数更新等设备差异。

### 13.3 `NPUGraphRunner.__init__`：先确定 bucket，再分配固定地址

[`DecodeCudaGraphRunner.__init__()`](https://github.com/sgl-project/sglang/blob/9a03bebf13996b628f8335628a691dcb3aa8400b/python/sglang/srt/model_executor/runner/decode_cuda_graph_runner.py#L191-L411) 会做四件与 GLM 最相关的事。
...
`captured_req_width=1` 表示每条活跃请求在一次普通 decode 中输入一个新 token。它不是 batch size，也不是 sequence length。若启用 speculative verify，同一请求一次可能放入多个候选 token，这个宽度才会大于 1。

#### 13.3.2 生成 batch-size buckets

`get_batch_sizes_to_capture(...)` 得到离散的 `capture_bs`，例如：

```text
capture_bs = [1, 2, 4, 8]
```

这里的 `bs` 是 **batch size**；每个离散 batch size 同时作为 batch-size bucket 的 key。真实 batch 为 3 时 padding 到 captured batch size 4，再复用 size-4 图。

#### 13.3.3 初始化 attention graph state
...
对普通 decode，`max_num_token == max_bs`。Ascend attention backend 在这里准备最大规格的 block table、FIA output/workspace、head padding 等 Graph 所需资源。

#### 13.3.4 创建最大静态输入 buffer

`DecodeInputBuffers.create(...)` 和 `CudaGraphBufferRegistry` 为以下内容准备稳定 storage：
...
seq_lens
out_cache_loc
block_tables / attention metadata
logits output buffer
其他 sampling、TP/DP 或 speculative metadata
```

不同 bucket 使用这些最大 buffer 的前缀 view。Capture 和 replay 之间可以改变 storage 中的值，但绑定给图的 `data_ptr` 不能随意换掉。

### 13.4 Warmup、Capture 和“加载”到底是什么关系

这是整条链中最容易混淆的一段。

#### 13.4.1 通用 warmup 与 NPU 每个 shape 的 warmup

[`DecodeCudaGraphRunner.capture()`](https://github.com/sgl-project/sglang/blob/9a03bebf13996b628f8335628a691dcb3aa8400b/python/sglang/srt/model_executor/runner/decode_cuda_graph_runner.py#L825-L873) 开头调用 `self.warmup()`。在固定提交中，通用 `BaseRunner.warmup()` 的 FlashInfer/DeepGEMM 逻辑主要针对 CUDA，NPU 会提前返回。

但这不代表 NPU 没有 warmup。真正的 NPU full-graph backend [`NPUCudaGraphBackend.capture_one()`](https://github.com/sgl-project/sglang/blob/9a03bebf13996b628f8335628a691dcb3aa8400b/python/sglang/srt/hardware_backend/npu/graph_runner/npu_cudagraph_backend.py#L78-L134) 对**每个 shape**先执行两次：

```python
for _ in range(2):
    torch.npu.synchronize()
    tp_group.barrier()
    forward_fn()                 # 正常执行，不在 capture context 内
    post_warmup_hook_if_needed()
```

这些 warmup forward 的作用包括：

- 让 lazy kernel/module/runtime 初始化发生在 capture 外；
- 让可能的 JIT/torch.compile 变体先生成或命中缓存；
- 让 workspace、allocator 和通信资源进入稳定状态；
- 让所有 TP ranks 在 capture 前保持相同阶段。

Warmup **会执行计算**，但不保存 task 序列为这张 Graph；其输出只是虚拟 capture batch 的临时结果。

#### 13.4.2 第三次 forward 才在 `torch.npu.graph` 中 capture

两次 warmup 后，backend 创建：

```python
graph = torch.npu.NPUGraph()
```

然后第三次调用同一个 `forward_fn()`：
...
这一次才把 NPU stream 上提交的可捕获 task、依赖和地址合同记录进 `graph`。`out` 是绑定 capture-time output storage 的 Python tensor/`LogitsProcessorOutput` 句柄；以后 replay 会让同一 storage 得到新内容。

`auto_dispatch_capture=True` 还让 torch_npu 为可更新的 fused infer attention task 保存 dispatch record，使 `actual_seq_lengths_kv` 能在 replay 前由 `NPUGraph.update(...)` 修改。详细实现见[第 05 讲 7.3 节](../05-npu-graph-and-compilation.md#73-replay_with_input_update动态-seq_lens-到底怎样更新)。

#### 13.4.3 为什么从大 bucket 向小 bucket capture

[`_capture_one_stream()`](https://github.com/sgl-project/sglang/blob/9a03bebf13996b628f8335628a691dcb3aa8400b/python/sglang/srt/model_executor/runner/decode_cuda_graph_runner.py#L875-L911) 使用：
...
若 `capture_bs=[1,2,4,8]`，顺序是 `8 -> 4 -> 2 -> 1`。大图先建立较大的 graph memory pool，小图更容易复用已分配的 pool，减少碎片和额外保留内存。

所以启动日志中的：

```text
Capturing batches (bs=8 ...)
Capturing batches (bs=4 ...)
...
```

不是正在处理八个真实用户请求，而是在用 dummy `ForwardBatch` 构造 size-8 的可复用执行规格。

#### 13.4.4 可选 `torch.compile` 在哪里插入

每个 bucket capture 前，runner 判断 `bs in compile_bs`。若成立，NPU 的 `patch_model_npu()` 会把 `model.forward` 包装为：
...
随后两次 NPU warmup 调用这个 compiled callable，触发编译并使其稳定；再在 `torch.npu.graph` context 中执行 capture。这里存在两个不同层次：

```text
torch.compile graph:
  Python/PyTorch program 的编译表示

torch.npu.NPUGraph:
  某个固定 bucket 下的 Device task launch graph
```

启用 compile 时二者叠加，但不是同一张图。详细区别见[第 05 讲第 9 节](../05-npu-graph-and-compilation.md#9-torchcompile-与-npugraph-的关系)。

### 13.5 一次 capture 怎样真实走过 GLM-4.7-Flash

以 captured batch size 4 为例，[`capture_one_shape()`](https://github.com/sgl-project/sglang/blob/9a03bebf13996b628f8335628a691dcb3aa8400b/python/sglang/srt/model_executor/runner/decode_cuda_graph_runner.py#L913-L1011) 先调用 `capture_prepare(4)`，从静态 buffer 的前缀 view 构造 dummy `ForwardBatch`：
...
这是“形状和地址真实、用户语义虚构”的 capture batch。它的作用是让模型完整跑通 size-4 decode 路径，并让所有会参与真实 replay 的算子出现在图里。

Runner 随后构造 `run_once()`，真实调用形式是：
...
对 GLM-4.7-Flash，这次调用按源码进入：

```text
Glm4MoeLiteForCausalLM.forward
  -> Glm4MoeLiteModel.forward
     -> embed_tokens
     -> 47 × Glm4MoeLiteDecoderLayer.forward
        -> RMSNorm
        -> DeepseekV2AttentionMLA
           -> q/kv projection + RoPE
           -> 写 paged MLA KV cache
           -> AscendAttnBackend.forward_decode_graph
           -> graph-compatible FIA .out
           -> absorbed value projection + o_proj
        -> Glm4MoeLiteSparseMoeBlock / dense MLP
        -> residual
     -> final RMSNorm
  -> LogitsProcessor + TP vocab projection/gather
  -> LogitsProcessorOutput
```

模型入口可以从 [`Glm4MoeLiteForCausalLM.forward()`](https://github.com/sgl-project/sglang/blob/9a03bebf13996b628f8335628a691dcb3aa8400b/python/sglang/srt/models/glm4_moe_lite.py#L968-L1000) 逆向跟进 model、decoder layer 和 attention。`forward_batch.forward_mode=DECODE` 以及 capture/graph context 让 Ascend attention 选择 graph-compatible 分支；因此捕获的不是第 12 节 eager `_npu_paged_attention_mla`，而是后文 13.7～13.8 的 FIA `.out` 路径。

在 TP=4、captured batch size 4 的一个 rank 上，关键 shape 可以粗略追成：

```text
input_ids                  [4]
embedding hidden           [4, 2048]

每个 decoder layer:
  q latent / q heads       由第 9、12 节的 MLA 投影生成
  local real Q heads       5
  graph-padded Q heads     8
  paged latent KV          每 token 保存 512 + 64 维
  attention output         [4, 5, 512] -> [4, 5, 256]
  o_proj / residual        [4, 2048]
  MoE output / residual    [4, 2048]

final logits               [4, vocab] 或绑定的 logits buffer view
```

Graph 记录的是这些模块在 capture stream 上最终发出的 NPU task 与依赖，包括 ATen NPU kernel、显式 `torch_npu/npu_*` op、可捕获通信以及 FIA dispatch task。它不会把 Python 的 `for layer in self.layers` 循环对象原样保存；循环已在 capture 时执行完，replay 只重放由它产生的 Device task 序列。

每个 bucket 完成后，NPU backend 保存：
...
- `_graphs` 是进程内 `ShapeKey -> NPUGraph` 字典；
- `_outputs` 保存固定输出 storage 的 Python handle；
- `shape_key` 至少区分 captured batch size，还可能包含 stream/variant 等规格；
- 权重不被复制进字典，图中的 kernel 仍读取本 rank 已加载权重的稳定地址。

到这里所谓“图加载完成”，更准确的表述是：**当前 TP rank 已现场捕获并持有该 bucket 的 runtime graph object，等待在线请求命中。**

### 13.6 第一个真实 decode 怎样命中并 replay

先看请求时间线。Prompt 的 prefill 已经把历史 latent KV 写入 paged cache；接下来 Scheduler 为每条未完成请求组织一个新 token，形成 decode `ForwardBatch`。[`ModelRunner._forward_raw()`](https://github.com/sgl-project/sglang/blob/9a03bebf13996b628f8335628a691dcb3aa8400b/python/sglang/srt/model_executor/model_runner.py#L1479-L1517) 检查：
...
if can_run_graph:
    return self.decode_cuda_graph_runner.execute(forward_batch, ...)
```

`is_cuda_graph()` 仍是兼容命名；NPU 下它表示当前 forward mode 允许进入 device graph。`can_run_graph()` 还要确认 live batch 的 shape、动态特性和 variant 能映射到已捕获 `ShapeKey`。
...
Runner 把 live batch padding 到 4，并写入 capture 时的静态 storage：

```text
static input_ids       [token_A, token_B, token_C, pad]
static positions       [10,      25,      7,       pad]
static seq_lens        [11,      26,      8,       fill]
static out_cache_loc   [slot_A,  slot_B,  slot_C,  safe_pad_slot]
actual_seq_lengths_kv  [11,      26,      8,       0]
```

第 4 行不是第四条真实请求，而是 size-4 图要求的 padding slot。Graph 执行固定的四行规格，返回后只保留前三行有效结果。

#### 13.6.2 两类动态输入使用不同更新机制
...
在 GLM 的 MLA 路径里，`attr_name` 会被解析为 `"actual_seq_lengths_kv"`，也就是 torch_npu fused infer attention 算子里表示“真实 KV 序列长度”的 Host 侧 keyword。这里的 `attr_type` 不是 Python 的 `list` 类型对象，也不是 `torch.dtype`；源码当前用空 list `[]` 作为一个约定标记，表示这个属性应该继续以普通 Host list 的形式传给算子。因此 backend 不会把 `seq_lens` 转成 tensor，而是保留为 `list[int]`，并在 replay 前组装成如下 `cpu_update_input`：
...
例如本例捕获规格为 `Bg=4`、当前真实 decode batch 为 `B=3`：
...
hidden_states  [4,2048]
```

前 3 行是有效请求，最后 1 行是 padding。replay 返回后，`NPUGraphRunner` 只保留 `raw_num_token/raw_bs` 对应的有效 logits 或 hidden states。

#### 13.6.3 replay 返回 output handle 时，Device 可以仍在执行

`graph.replay()` 是向 NPU stream 提交已记录 task；返回的 `_outputs[shape_key]` 是固定 storage 的 tensor handle。Host 不需要等所有 NPU 计算完成才拿到这个 handle。后续 sampler 在同一条有序 stream 上读取 logits 时，会排在写 logits 的 Graph task 之后；若 CPU 真正读取结果，则 D2H copy/Event/synchronize 建立 Host 可见性。

因此 Python 拿到 handle 不等于 Python 已经读出了尚未完成的元素值。handle 描述“哪块 storage”，stream/event 描述“什么时候可以消费其中的新内容”。完整解释见[第 05 讲第 7.4 与第 8 节](../05-npu-graph-and-compilation.md#74-为什么-replay-返回旧的-self_outputsshape_key)。

#### 13.6.4 启动和在线执行拼成一张时序图
...
sequenceDiagram
  ...
  Note over S,N: "服务启动阶段"
  S->>M: "加载本 rank 权重"
  S->>M: "创建 KV pool 与 Ascend attention backend"
  S->>R: "创建 NPUGraphRunner"
  R->>R: "分配最大静态 buffer，生成 capture_bs"
  loop "bs 从大到小"
    R->>B: "capture_one(shape_key, forward_fn)"
    B->>M: "warmup forward #1"
    B->>M: "warmup forward #2"
    B->>N: "进入 torch.npu.graph context"
    B->>M: "capture forward：embedding -> 47 layers -> logits"
    N-->>B: "NPUGraph runtime object"
    B->>B: "保存 _graphs[key] 与 _outputs[key]"
  end

  Note over S,N: "在线 decode 阶段"
  S->>M: "ForwardBatch(raw_bs=3)"
  M->>R: "can_run_graph -> execute"
  R->>R: "选择 bs=4，copy live 值到静态 buffer"
  R->>B: "update actual_seq_lengths_kv + replay"
  B->>N: "重放 size-4 task 序列"
  B-->>M: "返回固定 logits handle"
  M->>N: "sampler 排在同一 stream 后续"
  N-->>S: "next token 经 D2H/Event 变为 Host 可见"
```

### 13.7 为什么 Graph attention 不走 `_npu_paged_attention_mla`
