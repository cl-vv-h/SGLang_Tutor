...
对 GLM 的 MLA 映射，`attr_name` 是 `"actual_seq_lengths_kv"`，`attr_type` 当前是空 list `[]` 这个“目标表示标记”，不是 Python `list` 类型对象，也不是 `torch.dtype`。Backend 因而保留 `seq_lens` 为 `list[int]`，包装成：
