...
**简体中文** | [English](../../../en/ai-infra-basic/Speculative_Decoding/README.md)
...
本专题里的公式尽量按“先语义、再表达式、最后解释”的方式阅读：
...
```text
如果 draft 提出的 token y 在 target 眼里也足够合理，就接受；
如果 draft 对 y 太自信，而 target 没那么认可，就只按比例接受；
接受概率最高不能超过 1。
```
...
```text
平均提交 token 数 = 平均接受的 draft token 数 + 1 个修正/奖励 token
```

而 target verify 的成本没有线性增长到 `K` 倍，就能减少大模型调度轮数，降低 inter-token latency 并提高吞吐。
...
2. 接受概率 `min(1, p(y)/q(y))` 为什么表示“target 认可 draft 的程度”，以及为什么能保持 target distribution。
