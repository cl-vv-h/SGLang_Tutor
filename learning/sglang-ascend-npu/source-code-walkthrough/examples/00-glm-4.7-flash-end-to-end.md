...
读源码时可以用一句话判断这类分支：只要看到 `shape[0] > 0`，先问“当前 rank 当前阶段有没有真实 token 行？”而不是问“模型 batch size 为什么为 0？”这会让第 9.4 节的 MoE 分支好理解很多。
...
#### 9.4.4 TBO 拆分 op 与普通 forward 的对应关系
...
10. `Glm4MoeLiteSparseMoeBlock.forward_normal()`、`forward_normal_dual_stream()`、`forward_deepep()` 的触发条件和本质区别是什么？
11. NPU 上 routed expert 的五个主要算子步骤是什么？
12. prefix cache 命中后，为什么需要再次执行 `kv_b_proj`？
13. eager 正确但 graph 错误时，哪些 metadata 必须逐项比较？
14. `Glm4MoeLiteForCausalLMNextN` 如何同时使用 token embedding 和 target hidden state？
15. `glm45` reasoning parser 与 `glm47` tool parser 位于模型 forward 之前还是之后？
