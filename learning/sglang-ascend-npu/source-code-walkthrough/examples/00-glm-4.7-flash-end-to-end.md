...
下图使用 Graphviz 绘制，而不是 Mermaid。它同时展示模型对象树、单层 decoder 的内部结构、TP=4 的关键权重形状，以及 prefill/decode 两条不同的 MLA 执行路径。SVG 是矢量图，可以放大查看文字和连线。

![GLM-4.7-Flash 在 SGLang Ascend NPU 中的完整模型架构](../assets/glm-4.7-flash-architecture.svg)

[查看可编辑的 Graphviz DOT 源文件](../assets/glm-4.7-flash-architecture.dot)
...
这一节先给一句结论：**checkpoint 中的参数名字和数学结构偏向“便于训练和发布”，运行时参数偏向“少发起算子、适合 TP 分片、适合 NPU kernel 读取”。权重加载就是二者之间的结构化搬运。**
...
阶段 B：load_weights 遍历 checkpoint tensors
...
| 要检查的对象 | 检查内容 |
|---|---|
| checkpoint tensor | 名字、global shape、dtype |
| SGLang parameter | 运行时名字、local shape、所属 TP rank |
| parameter 的 `weight_loader` | 取 checkpoint 的哪一片、写到目标参数的哪一段 |
...
#### 4.8.3 checkpoint 名字怎样落入融合参数

`load_weights()` 遍历 checkpoint 的 `(name, loaded_weight)`。遇到：

```text
...mlp.gate_proj.weight [10240,2048]
...mlp.up_proj.weight   [10240,2048]
```
...
每次加载还会先从 global checkpoint 权重取当前 TP rank 对应的 2560 行。以 rank 2 为例，概念上取全局输出行 `[5120:7680]`；然后分别写入 local packed 参数的 `[0:2560]` 和 `[2560:5120]`。实际切片由参数绑定的 `weight_loader` 完成，不应在模型代码外手工切权重。
...
每个 routed expert 在 checkpoint 中仍是三块独立权重：
...
| 运行时参数 | TP4 local / NPU physical shape | 来自 checkpoint | 使用位置 |
|---|---:|---|---|
...
普通 target model 加载时会跳过 checkpoint 中第 47 层的 NextN 参数；启用 speculative decoding 后，它们由 `Glm4MoeLiteForCausalLMNextN` 作为 draft model 单独加载。这样 target 的 47 层和 NextN 的 1 层不会落入错误的对象。
...
本讲的完整模型图见第 2.3 节：[GLM-4.7-Flash Graphviz 架构图](../assets/glm-4.7-flash-architecture.svg)。这里不再放一张信息更少的 Mermaid 图，而是把图上的区域直接映射到代码入口。
