...
| `layers/moe/fused_moe_triton/layer.py` | `FusedMoE.make_expert_params_mapping()` |
...
`params_dict` 只包含当前进程实际持有的 parameter。PP 下不属于当前 stage 的层会是 `PPMissingLayer`，EP 下不属于当前 rank 的 expert 也可能不存在；因此“输入权重存在但 `name` 不在 `params_dict`”经常表示该权重不属于当前 rank，而不一定是错误。

#### 4.8.5 每个输入 tensor 的真实分支优先级
...
#### 4.8.6 `gate_proj` 和 `up_proj` 的“合入”到底是什么
...
#### 4.8.8 Q latent 和 KV latent 为什么也要融合
...
#### 4.8.9 expert 权重怎样打包成 `w13_weight` 和 `w2_weight`
...
#### 4.8.11 `kv_b_proj` 怎样派生出 `w_kc` 和 `w_vc`
...
#### 4.8.12 `quant_method.process_weights_after_loading()` 做什么
...
#### 4.8.14 用具体 `name` 走一遍完整加载过程

下面按模型结构列出代表性权重，但不假设它们在文件中严格按此顺序出现。每一行都可以独立套用第 4.8.5 节的分支优先级。
