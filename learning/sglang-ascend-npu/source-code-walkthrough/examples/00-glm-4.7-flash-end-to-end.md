...
shared expert 在 Ascend 基线中复用第 9.2 节的 `F.linear → npu_swiglu → F.linear`，并以 `reduce_results=False` 构造。routed/shared partial 先使用普通 `torch.add` 的 NPU kernel 在本 rank 相加，再通过 HCCL AllReduce 或 reduce-scatter 统一完成跨 rank 汇总，避免两条分支分别通信。
...
final norm 继续使用第 9.1 节的显式 NPU 算子：有 residual 时调用 `torch_npu.npu_add_rms_norm`，否则调用 `torch_npu.npu_rms_norm`。
