# -*- coding: utf-8 -*-
#
# 教学版文件：由 python/sglang/srt/managers/scheduler.py 生成。
# 用途：在不修改原始源码的前提下，为 Scheduler 源码阅读提供中文块级注释。
# 注意：本文件只用于学习对照，不参与 SGLang 运行；如需运行请使用原始 scheduler.py。
#
...
if is_mps():
    ...
    # 教学注释：SchedulerMlxOverlapMixin 是本文件中的类定义，封装 Scheduler 相关的一组状态和行为。
...
# 教学注释：Scheduler 是 SGLang 运行时调度中枢，负责接收请求、维护队列、组装 ScheduleBatch、调用模型 worker，并协调 KV cache、grammar、LoRA、overlap、disaggregation 和 metrics。
class Scheduler(
    SchedulerDisaggregationDecodeMixin,
    SchedulerDisaggregationPrefillMixin,
    ...
    # 教学注释：初始化 Scheduler 的全部运行时依赖，包括并行状态、模型配置、IPC、model worker、KV cache、调度策略、grammar、LoRA、overlap 与监控组件。
    def __init__(
        self,
        ...
        # 教学注释：cur_batch 记录当前轮正在调度或执行的 batch，结果处理和 pause/abort 逻辑会读取它。
        ...
        # 教学注释：这里构建 KV cache 体系。KV cache 不只是显存块，还包括 request slot、token slot、prefix/radix tree cache 和可选的层级缓存。
    ...
    # 教学注释：init_zbal_on_npu 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：加载并标准化模型配置
    # 教学注释：后续 context length、dtype、多模态判断、KV cache 容量和 worker 初始化都依赖它。
    ...
    # 教学注释：建立 Scheduler 与 tokenizer、detokenizer、RPC 等进程之间的 IPC 通道，是请求输入和结果输出的通信底座。
    ...
    # 教学注释：init_idle_sleeper 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：publish_load_snapshot 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：handle_get_loads_req 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    def handle_get_loads_req(self, req: GetLoadsReqInput):
        return self.load_inquirer.get_loads(req)

    # 教学注释：init_tokenizer 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_mamba_backend 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    def init_mamba_backend(self) -> None:
        initialize_mamba_selective_state_update_backend(self.server_args)

    # 教学注释：init_moe_gemm_config 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_tp_model_worker 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：maybe_init_draft_worker 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：初始化真正执行 forward 的模型 worker，并按配置创建主模型、draft worker 或其它执行后端。
    ...
    # 教学注释：初始化调度循环中的核心可变状态，例如 waiting_queue、running_batch、cur_batch、last_batch、健康检查队列和暂停标记。
    def init_running_status(self):
        # 教学注释：waiting_queue 是新请求进入 prefill 前的主队列，后续 _get_new_batch_prefill_raw 会从这里挑选请求。
        self.waiting_queue: List[Req] = []
        # The running decoding batch for continuous batching
        # 教学注释：running_batch 表示已完成 prefill、正在或即将 decode 的请求集合，是 continuous batching 的核心状态。
        self.running_batch: ScheduleBatch = ScheduleBatch(reqs=[], batch_is_full=False)
        # The current forward batch
        self.cur_batch: Optional[ScheduleBatch] = None
        # The last forward batch
        # 教学注释：last_batch 连接上一轮 forward 和下一轮调度
        # 教学注释：prefill 完成后通常在下一轮合并进 running_batch。
        self.last_batch: Optional[ScheduleBatch] = None
        self.forward_ct = 0
        # 教学注释：忙碌时健康检查请求不会进入完整生成链路，而是暂存在这里，等一次结果处理后返回轻量信号。
        ...
        # 教学注释：暂停标记控制事件循环是否继续推进 GPU forward
        # 教学注释：控制请求仍可被接收和处理。
        self._engine_paused = False

    # 教学注释：初始化 chunked prefill
    # 教学注释：长 prompt 可以跨多轮 prefill，避免单轮占用过多 token 预算。
    def init_chunked_prefill(self):
        ...
        if (
            ...
            self.chunked_prefill_size = None
        # 教学注释：chunked_req 保存跨多轮 prefill 的长 prompt 请求
        # 教学注释：每轮只处理其中一个 chunk。
    ...
    # 教学注释：初始化调度策略、prefill delay 和 token ratio tracker，决定 waiting_queue 里的请求如何排序和插入。
    ...
    # 教学注释：init_soft_watchdog 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_watch_dog_memory_saver_input_blocker 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：初始化 prefill/decode disaggregation 相关队列和传输组件
    # 教学注释：PD 模式下请求会流经 bootstrap、prealloc、transfer 等专用路径。
    ...
    # 教学注释：初始化 overlap schedule 所需的 CUDA stream、future_map、result_queue 和张量生命周期保护结构。
    def init_overlap(self):
        ...
        if use_mlx():
            ...
            # 教学注释：result_queue 是 overlap 模式的结果缓冲区，保存已 launch 但尚未处理的 forward 结果。
    ...
    # 教学注释：maybe_init_ngram_embedding 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：_maybe_prepare_ngram_embedding 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_deterministic_inference_config 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：建立请求类型到 handler 的映射，让主循环只负责分发，不把所有请求类型判断塞进 event loop。
    def init_request_dispatcher(self):
        # 教学注释：TypeBasedDispatcher 把请求类型映射到 handler，避免主循环堆满 if/elif。
    ...
    # 教学注释：扫描 running_batch 中运行过久的请求，超过限制时设置 abort，避免长期占用 GPU/cache 资源。
    ...
    # 教学注释：get_init_info 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：Scheduler 长期服务循环入口：创建调度 stream，然后根据配置分发到 normal、overlap、PP 或 disaggregation event loop。
    ...
    @DynamicGradMode()
    # 教学注释：普通事件循环：收请求、处理输入、选择 batch、执行 forward、处理结果
    # 教学注释：没有可跑 batch 时进入 idle 维护。
    def event_loop_normal(self):
        """A normal scheduler loop."""
        while True:
            # Receive requests
            # 教学注释：每轮事件循环先从 IPC 批量读取请求
            # 教学注释：这些请求可能是生成、embedding 或控制命令。
            recv_reqs = self.request_receiver.recv_requests()
            # 教学注释：输入处理会把外部请求转换成内部状态变化，例如创建 Req、入队、abort、flush 或 RPC 响应。
            self.process_input_requests(recv_reqs)
            if self._engine_paused:
                continue

            # Get the next batch to run
            # 教学注释：这是每轮调度的核心调用，决定下一次 GPU forward 是 prefill、decode 还是无事可做。
            batch = self.get_next_batch_to_run()
            self.cur_batch = batch

            # Launch the current batch
            if batch:
                # 教学注释：调度决策完成后，run_batch 把 ScheduleBatch 交给模型 worker 执行。
                result = self.run_batch(batch)
                # 教学注释：forward 完成后必须把结果落回请求状态，并把 token/logprob/finish 信息发回上游。
    ...
    @DynamicGradMode()
    # 教学注释：overlap 事件循环：让本轮 GPU forward 与上一轮结果处理/下一轮调度准备重叠，减少 GPU 等 CPU 的时间。
    def event_loop_overlap(self):
        ...
        # 教学注释：pop_and_process 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
        # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
        def pop_and_process():
        ...
        while True:
            ...
            if batch:
                ...
                # 教学注释：overlap 模式下保存 batch 副本和结果，确保后续处理看到的是 launch 时的 batch 状态。
            ...
            if self.is_generation:
                # 教学注释：延迟采样要等上一轮结果处理完成后执行，因为 grammar 等状态可能依赖上一轮输出。
    ...
    # 教学注释：判断某些 batch 是否必须临时关闭 overlap，例如连续 prefill 或 spec+grammar decode 需要更严格同步。
    ...
    # 教学注释：处理从 IPC 收到的一批请求：健康检查短路、类型分发、立即响应控制请求、检查 pending flush 和外部 corpus 加载。
    def process_input_requests(self, recv_reqs: List):
        now = time.monotonic()
        self.session_controller.maybe_reap(now)
        for recv_req in recv_reqs:
            # Skip health check when server is busy — ongoing requests already carry health info.
            # 教学注释：健康检查请求在忙碌时走轻量路径，避免被长 prefill 阻塞。
            ...
            # 教学注释：具体请求类型在这里分派，例如 generate、abort、flush、LoRA、RPC 等。
    ...
    # 教学注释：init_profiler 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_weight_updater 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_lora_drainer 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_lora_overlap_loader 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_grammar_manager 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    def init_grammar_manager(self) -> None:
        self.grammar_manager = GrammarManager(self)

    # 教学注释：init_request_receiver 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_dp_attn_adapter 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_pool_stats_observer 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_invariant_checker 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_kv_events_publisher 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_load_inquirer 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_output_streamer 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_batch_result_processor 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_req_max_new_tokens 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：_process_and_broadcast_mm_inputs 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：_get_multimodal_inputs 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    @staticmethod
    # 教学注释：_try_apply_padded_mm_input_ids 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：_maybe_compute_mrope_positions 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：_maybe_clear_mm_inputs 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：生成请求入口：把 TokenizedGenerateReqInput 转为内部 Req，并处理 session、多模态、input_embeds、长度、logprob、grammar 和 disaggregation bootstrap。
    def handle_generate_request(
        ...
        if session_id is None:
            ...
            # 教学注释：Req 是 Scheduler 内部请求对象，聚合输入 token、采样参数、cache 索引、输出状态和完成原因。
            req = Req(
                recv_req.rid,
                recv_req.input_text,
                recv_req.input_ids,
        ...
        # 教学注释：结构化输出请求可能需要等待 grammar 准备
        # 教学注释：未 ready 时先进入 grammar 队列，不直接进入 waiting_queue。
        added_to_grammar_queue = self.grammar_manager.process_req_with_grammar(req)
        if not added_to_grammar_queue:
            self._add_request_to_queue(req)

    # 教学注释：批量生成请求入口，逐个调用 handle_generate_request，使 batch 请求复用单请求路径。
    ...
    # 教学注释：启用 HiCache storage 时提前触发 KV cache 预取，让请求 prefill 前尽量复用已有 prefix/cache。
    ...
    # 教学注释：请求进入调度系统的统一入口
    # 教学注释：普通模式进入 waiting_queue，PD prefill 进入 bootstrap，PD decode 进入 prealloc。
    def _add_request_to_queue(self, req: Req, is_retracted: bool = False):
        ...
        if self.disaggregation_mode == DisaggregationMode.NULL:
            ...
            # 教学注释：普通模式下，请求最终进入 waiting_queue，等待 prefill 调度。
    ...
    # 教学注释：设置或校验请求优先级，为 priority scheduling 和抢占逻辑提供排序依据。
    ...
    # 教学注释：等待队列过长时拒绝或 abort 新请求，避免服务接受超出自身承载能力的排队量。
    ...
    # 教学注释：扫描等待过久的请求并主动 abort，避免请求无限期停留在 waiting_queue。
    def _abort_on_waiting_timeout(self):
        ...
        deleted_reqs = set()
        deadline = time.perf_counter() - timeout_s
        # 教学注释：遍历等待队列，逐个尝试加入本轮 prefill batch
        # 教学注释：资源限制可能让循环提前停止或跳过请求。
    ...
    # 教学注释：处理 embedding 请求
    # 教学注释：它同样构造 Req 和 ScheduleBatch，但 forward 与结果输出不同于生成路径。
    ...
    # 教学注释：handle_batch_embedding_request 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：把未完成的 chunked request 暂存到 tree_cache，便于下一轮继续 prefill 时复用前缀状态。
    def stash_chunked_request(self, req: Req):
        maybe_cache_unfinished_req(req, self.tree_cache, chunked=True)

    # 教学注释：HiSparse 模式下把 staging 完成的请求组装成 decode batch，并把最后 token 放入 future_map。
    def _build_hisparse_decode_batch(self, reqs):
        ...
        # 教学注释：future_map 用于跨轮传递 next token 或 draft 输入，让下一轮调度不必立刻同步所有 GPU 张量。
    ...
    # 教学注释：Scheduler 的核心决策函数：先合并上一轮 prefill，再优先调度新 prefill
    # 教学注释：没有 prefill 时推进 running_batch decode。
    def get_next_batch_to_run(self) -> Optional[ScheduleBatch]:
        ...
        if (
            ...
            # 教学注释：合并上一轮 prefill 前先过滤完成、abort 或 chunked 特殊请求，避免错误进入 running_batch。
            ...
            if not self.last_batch.is_empty():
                if self.running_batch.is_empty():
                    ...
                    # 教学注释：prefill 完成后，未结束请求进入 running_batch，之后由 decode 循环持续推进。
        ...
        if self.dllm_config is not None:
            ...
            # 教学注释：Scheduler 优先尝试插入新的 prefill，这是 continuous batching 能接纳新请求的关键。
        ...
        if new_batch is not None:
            ...
            if (
                ...
                # 教学注释：没有新 prefill 可跑时，更新 running_batch 并准备 decode。
    ...
    # 教学注释：get_num_allocatable_reqs 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：构造 prefill batch 的包装层，负责接入 PrefillDelayer，再调用 _get_new_batch_prefill_raw 做实际选择。
    ...
    # 教学注释：真正选择 prefill 请求的核心函数
    # 教学注释：处理 grammar ready、HiCache 事件、priority、chunked prefill、LoRA、KV/request 预算和抢占。
    def _get_new_batch_prefill_raw(
        ...
        # Get priority queue
        # 教学注释：调度策略在这里为等待请求计算优先级，影响 PrefillAdder 遍历顺序。
        ...
        # Prefill policy
        # 教学注释：PrefillAdder 是 prefill 预算控制器，负责判断每个请求能否放进当前 prefill batch。
        adder = PrefillAdder(
            self.page_size,
            self.tree_cache,
        ...
        if self.chunked_req is not None:
            ...
            # 教学注释：如果上一轮留下 chunked prefill 请求，本轮优先尝试继续推进它。
        ...
        for req in self.waiting_queue:
            ...
            # 教学注释：这里真正尝试把一个 Req 加入 can_run_list，同时更新 token 预算、prefix cache 命中和 chunked 状态。
        ...
        # 教学注释：已经选入 can_run_list 的请求从 waiting_queue 移除，避免重复调度。
        ...
        # Create a new batch
        # 教学注释：把 can_run_list 转换成 ScheduleBatch，绑定 request pool、KV pool、tree cache 和 forward 元信息。
        ...
        # 教学注释：为 prefill/extend forward 准备张量、seq_lens、cache 写入位置和 sampling 信息。
        ...
        if (
            ...
            # 教学注释：decode 前先过滤已完成或 abort 的请求，防止它们继续占用 batch。
            self.running_batch.filter_batch(v1_spec_info_filtered=True)
            if not self.running_batch.is_empty():
                # 教学注释：为 decode forward 准备 next token 输入、seq_lens 和采样状态。
                self.running_batch.prepare_for_decode()
                # 教学注释：mixed chunked prefill 可以把新 prefill 与已有 decode 混合成一个 batch，提高资源利用率。
    ...
    # 教学注释：判断 LoRA 请求能否加入当前 batch，既要看 drainer 状态，也要看 adapter 是否已加载或还能加载。
    ...
    # 教学注释：更新 decode 阶段的 running_batch：过滤完成请求、检查 decode KV cache、必要时 retract 请求并重新入队，最后准备 decode 张量。
    def update_running_batch(self, batch: ScheduleBatch) -> Optional[ScheduleBatch]:
        ...
        # 教学注释：decode 每轮都会继续写 KV cache
        # 教学注释：空间不足时必须撤回部分请求释放显存。
        if (kv_full_retract_flag := not batch.check_decode_mem()) or (
            ...
            # 教学注释：retract_decode 会选择要撤回的请求，释放其 KV cache，并返回新的 token ratio 估计。
    ...
    # 教学注释：overlap 模式下保存 batch 字段和张量引用，防止 GPU forward 仍在使用的数据被 Python GC 或调度侧改写。
    ...
    @contextmanager
    # 教学注释：overlap forward 的事务保护区，forward 前保存 ScheduleBatch 关键字段，forward 后恢复调度侧视图。
    ...
    # 教学注释：执行一次 ScheduleBatch
    # 教学注释：generation 调用 forward_batch_generation，embedding 调用 forward_batch_embedding，overlap 模式还要处理 future_map 和 CUDA stream。
    def run_batch(
        self,
        batch: ScheduleBatch,
        ...
        if self.is_generation:
            if self.enable_overlap:
                ...
                with self.forward_stream_ctx:
                    ...
                    # 教学注释：overlap 下 worker 可能改写 batch 字段，这个上下文保证调度侧视图能恢复。
                    with self._overlap_forward_isolation(batch):
                        ...
                        # 教学注释：真正的生成模型 forward 调用发生在这里
                        # 教学注释：Scheduler 到此已经完成 batch 选择和输入准备。
            ...
            if self.enable_overlap:
                ...
                with self.forward_stream_ctx:
                    self.forward_stream.wait_stream(self.schedule_stream)
                    resolve_forward_inputs(batch, self.future_map)
                    # 教学注释：embedding/reward 模型不走生成 token 路径，而是返回 embedding 或 pooled hidden states。
    ...
    # 教学注释：_maybe_report_active_ranks 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：处理延迟采样
    # 教学注释：某些 overlap/structured output 场景需要等上一轮结果处理完再执行 sampling。
    ...
    # 教学注释：forward 结果处理统一入口，根据 forward_mode 分流到 decode、prefill/extend、prebuilt、idle，并统一做 metrics、健康检查和资源清理。
    def process_batch_result(
        ...
        # 教学注释：decode 结果处理会追加新 token、判断完成、发送流式输出，并释放完成请求资源。
        if batch.forward_mode.is_decode():
            self.batch_result_processor.process_batch_result_decode(batch, result)
        # 教学注释：extend/prefill 结果处理负责首 token、logprob、prefix cache 插入，以及把未完成请求留给后续 decode。
        ...
        # 教学注释：每次结果处理后更新 batch 级统计，用于吞吐、延迟和调度指标。
        ...
        # 教学注释：结果处理结束后顺手返回之前挂起的健康检查信号。
        self.maybe_send_health_check_signal()
        self.metrics_reporter.update_device_timer()

    # 教学注释：忙碌期间缓存的健康检查 IPC 会在 batch 结果处理后返回轻量信号。
    ...
    # 教学注释：add_external_corpus 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：remove_external_corpus 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：list_external_corpora 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：clear_hicache_storage_wrapped 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：Scheduler 完全空闲时执行一致性检查、metrics 刷新、KV event 发布、token ratio reset 和 idle sleep。
    def on_idle(self):
    ...
    # 教学注释：判断 Scheduler 是否真正空闲
    # 教学注释：除 waiting/running 外，还检查 last/cur batch、overlap result_queue、grammar、disagg、HiCache 等状态。
    ...
    # 教学注释：attach_hicache_storage_wrapped 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：detach_hicache_storage_wrapped 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：清空 tree cache、request/token pool、grammar 和设备 allocator cache
    # 教学注释：只有 fully idle 时才安全。
    def flush_cache(self, empty_cache: bool = True):
        """Flush memory pools (e.g., KV cache, Mamba cache) and optionally empty device allocator cache."""
        # 教学注释：很多破坏性维护操作必须要求 fully idle，避免清掉仍被请求使用的 cache 或状态。
        if self.is_fully_idle():
            self.cur_batch = None
            self.last_batch = None
            # 教学注释：flush cache 时重置 prefix/radix/HiCache 状态。
    ...
    # 教学注释：get_internal_state 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：set_internal_state 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：save_remote_model 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    def save_remote_model(self, **kwargs):
        self.weight_updater.save_remote_model(kwargs)

    # 教学注释：save_sharded_model 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    def save_sharded_model(self, **kwargs):
        self.weight_updater.save_sharded_model(kwargs)

    # 教学注释：handle_rpc_request 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：取消请求：未开始的可直接移除，grammar/disagg 队列通知对应子系统，running 请求设置 FINISH_ABORT 等待统一清理。
    def abort_request(self, recv_req: AbortReq):
        ...
        for req in reqs:
            if not req.finished() and (
                ...
                # 教学注释：running 中的请求不能直接从 batch 删除，设置 FINISH_ABORT 后交给统一结果处理路径清理。
                req.to_finish = FINISH_ABORT()

    # 教学注释：_pause_engine 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    def _pause_engine(self) -> Tuple[List[Req], int]:
        raise NotImplementedError()

    # 教学注释：暂停生成
    # 教学注释：in_place 保留状态，retract 模式会把 running 请求撤回队列，便于权重更新等操作。
    def pause_generation(self, recv_req: PauseGenerationReqInput):
        # 教学注释：设置暂停标记后，事件循环仍会接收控制请求，但不会继续推进新的 GPU forward。
    ...
    # 教学注释：恢复生成，可选清理 CUDA allocator cache，然后解除 _engine_paused 标记。
    ...
    # 教学注释：load_lora_adapter 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：load_lora_adapter_from_tensors 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：unload_lora_adapter 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：init_weights_send_group_for_remote_instance 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：send_weights_to_remote_instance 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：slow_down 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：expert_distribution_handle 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：open_session 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：close_session 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    def close_session(self, recv_req: CloseSessionReqInput):
        self.session_controller.close(recv_req)

    # 教学注释：maybe_sleep_on_idle 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    def maybe_sleep_on_idle(self):
        if self.idle_sleeper is not None:
            self.idle_sleeper.maybe_sleep()

    # 教学注释：handle_freeze_gc 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：configure_logging 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：handle_dumper_control 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
    ...
    # 教学注释：update_cache_from_scheduler 是 Scheduler 的辅助代码块，负责处理一个局部运行时职责
    # 教学注释：阅读时重点看它读写了哪些状态，以及把控制流交给哪个下一跳。
...
# 教学注释：根据 disaggregation、PP、pdmux、overlap 等配置选择具体事件循环，决定 Scheduler 后续主循环模式。
def dispatch_event_loop(scheduler: Scheduler):
    # Dispatch to the appropriate event loop based on the disaggregation mode
    server_args = scheduler.server_args
    disaggregation_mode: DisaggregationMode = scheduler.disaggregation_mode
    # 教学注释：非 PD 分离模式使用本地 Scheduler 同时处理 prefill 和 decode。
    ...
    # 教学注释：PD prefill worker 只负责 prompt prefill 和 KV 发送。
    ...
    # 教学注释：PD decode worker 负责接收 KV 并持续 decode。
...
# 教学注释：配置 Scheduler 子进程，包括进程标题、日志、faulthandler、CPU affinity 和 NUMA。
...
# 教学注释：Scheduler 子进程入口：加载插件、配置进程、创建 Scheduler、上报初始化信息并进入事件循环。
def run_scheduler_process(
    ...
    scheduler = None
    try:
        # 教学注释：完成进程配置后创建 Scheduler 实例，此时会触发大量运行时组件初始化。
        scheduler = Scheduler(
            server_args,
            port_args,
            gpu_id,
        ...
        # 教学注释：初始化完成后进入阻塞事件循环，Scheduler 的服务生命周期主要停留在这里。
